# Delta for backend-lint-gate

Capacidad nueva: todo es ADDED. Alcance: `ruff.toml` (raíz), `backend/requirements-dev.txt` y el
job `lint` de `.github/workflows/ci.yml`.
Invariante rector: **el lint no afirma nada que no haya comprobado, y lo que comprueba se lee**.

## ADDED Requirements

### Requirement: BLG-001 — El lint del backend tiene configuración explícita
...
#### Scenario: La configuración existe y es enumerada
...
#### Scenario: El conjunto por defecto no es una configuración
...
### Requirement: BLG-002 — `Depends()` no es un hallazgo
...
#### Scenario: Los 63 siguen ahí y siguen siendo correctos
...
#### Scenario: Mutación — «arreglar» B008 debe rechazarse

- GIVEN alguien sustituye `def f(user: dict = Depends(get_current_user))` por un patrón que evite
...
### Requirement: BLG-003 — El backend está a cero y el job bloquea
...
#### Scenario: Cero medido sobre el árbol que se fusiona
...
#### Scenario: Un hallazgo nuevo para el merge
...
#### Scenario: Mutación — devolver `continue-on-error` debe notarse

- GIVEN se reintroduce `continue-on-error: true` en el job `lint`
...
### Requirement: BLG-004 — Una sola versión de ruff en todo el repositorio
...
#### Scenario: El desarrollador reproduce el CI
...
#### Scenario: Mutación — dos literales de versión

- GIVEN la versión aparece tanto en el workflow como en el fichero de dependencias de desarrollo
...
### Requirement: BLG-005 — Un `except` silencioso registra por qué
...
#### Scenario: El circuit breaker deja de fallar en silencio
...
#### Scenario: Un circuito que no puede recuperarse lo dice
...
#### Scenario: La caché de embeddings no es un incidente
...
#### Scenario: Mutación — quitar el log debe romper la suite

- GIVEN se devuelve `except Exception: pass` en la persistencia del circuito
- WHEN se ejecuta la suite
- THEN el primer escenario de este requisito **MUST** fallar

### Requirement: BLG-006 — Un import muerto y un re-export no son lo mismo
...
#### Scenario: Los re-exports sobreviven
...
#### Scenario: El catálogo no se vacía por un `--fix`
...
#### Scenario: Mutación — borrar el import de efecto secundario

- GIVEN se elimina de `load_all_tools()` el import de `oauth_tools`
- WHEN se ejecuta la suite
...
### Requirement: BLG-007 — Una fecha sin zona se corrige o se justifica
...
#### Scenario: Marcas de tiempo almacenadas
...
#### Scenario: El reloj del operador
