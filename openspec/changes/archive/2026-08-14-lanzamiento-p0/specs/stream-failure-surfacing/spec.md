# stream-failure-surfacing Specification
...
### Requirement: SFS-001 — El evento `error` se ve
...
#### Scenario: Error seguido de `[DONE]`
...
#### Scenario: Error a mitad de generación
...
#### Scenario: Mutación — devolver el `throw` al `try`

- GIVEN se restaura el `throw new Error(data.message …)` dentro del `try` cuyo
  `catch (parseError)` lo registra como error de parseo (`api.ts:221-226`)
...
### Requirement: SFS-002 — Fin de stream sin `[DONE]`
...
#### Scenario: Corte de conexión limpio
...
#### Scenario: Chunk ilegible entre chunks buenos
...
#### Scenario: Mutación — dejar de tolerar el chunk corrupto

- GIVEN se elimina el `catch` que descarta el chunk ilegible
- WHEN se ejecuta la suite
- THEN el test de chunk ilegible MUST fallar

### Requirement: SFS-004 — El fallo cierra el estado de carga
...
#### Scenario: Mutación — quitar la limpieza en `onError`

- GIVEN se elimina la salida del conjunto de sesiones en streaming dentro de `onError`
- WHEN se ejecuta la suite del store
- THEN el test del spinner MUST fallar

### Requirement: SFS-005 — Mensaje presentable
...
#### Scenario: Error sin mensaje del servidor
...
#### Scenario: Mutación — mostrar el error crudo

- GIVEN se sustituye el mensaje por `String(error)` de un objeto de error sin `message`
- WHEN se ejecuta la suite
