# acta-deliverable Specification
...
### Requirement: AD-001 — El acta sobrevive a la recarga
...
#### Scenario: El acta vuelve siendo acta
...
#### Scenario: Mutación — el backend cambia la etiqueta a escondidas

- GIVEN se cambia la plantilla del acta (`board_v2.py:466`) a `artifact_type="markdown"`
- WHEN se ejecuta la suite de backend
- THEN el test que fija la etiqueta emitida MUST fallar

#### Scenario: El literal del test es el mismo en los dos lados

- GIVEN los dos tests
- WHEN se comparan las cadenas que afirman
- THEN ambos afirman sobre `type="markdown"`, sin normalizar ni reescribir la etiqueta

### Requirement: AD-003 — Formatos de «Próximos pasos»
...
#### Scenario: Sub-encabezados dentro de la sección
...
#### Scenario: Mutación — restaurar el corte en cualquier encabezado

- GIVEN se restaura el `break` incondicional ante `HEADING_RE` (`actaParser.ts:55`)
- WHEN se ejecuta la suite
- THEN el test de sub-encabezados MUST fallar (devuelve 0 items)

### Requirement: AD-004 — Previsualización de títulos antes de crear issues
...
#### Scenario: Los títulos a la vista
...
#### Scenario: Mutación — volver al recuento a secas

- GIVEN se sustituye la lista de títulos por el texto «Se crearán {N} issues»
- WHEN se ejecuta la suite
