# Delta for n8n-webhook-ingress

Capacidad nueva: todo es ADDED. Cubre el endpoint público sin auth
(`POST /api/v1/webhooks/n8n`, `webhooks.py:469-503`) y **la semántica única del secreto
compartido** `N8N_WEBHOOK_SECRET`, que hoy tiene cuatro consumidores con comportamientos
distintos ante el secreto vacío.
...
### Requirement: NWI-001 — Secreto vacío = integración apagada, nunca «cualquier firma vale»
...
#### Scenario: Firma calculada con la clave vacía
...
#### Scenario: OAuth no arranca sin secreto
...
#### Scenario: Mutación — quitar la guarda

- GIVEN se elimina `if not secret: return False` de `verify_n8n_signature`, o se permite firmar el
...
#### Scenario: State no ASCII no provoca 500

- GIVEN `state = "abc:1700000000:ñ"`
- WHEN se verifica
- THEN devuelve `False` sin propagar `TypeError`

### Requirement: NWI-003 — Contrato observable del endpoint público
...
función pura (hoy `test_webhook_n8n.py` sólo ejercita `verify_n8n_signature`; el endpoint tiene
0 tests).
...
#### Scenario: `user_id` no string no llega a Mongo
...
#### Scenario: Mutación — quitar `isinstance(user_id, str)`

- GIVEN se elimina la guarda de `webhooks.py:445`
- WHEN se ejecuta la suite
- THEN el escenario anterior **MUST** fallar

### Requirement: NWI-004 — Replay protection con caducidad declarada
...
#### Scenario: Reenvío del mismo payload
...
#### Scenario: Fuera de ventana temporal

- GIVEN un payload con `timestamp` anterior a la ventana
- WHEN llega con firma correcta
- THEN se rechaza igualmente

#### Scenario: Mutación — desactivar el dedupe

- GIVEN se elimina la comprobación de `nonce`
- WHEN se ejecuta la suite
