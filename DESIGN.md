...
> **Enmienda (Viveza-2), al implementarlo.** Lo que se descubrió al ir a hacerlo:
>
> 1. **El estado de las herramientas no tenía dónde vivir.** No existía ningún almacén de ejecuciones: `onToolStart` y sus hermanos serializaban marcadores de texto (`[TOOL_START:…]`) DENTRO del contenido del mensaje, y un regex los volvía a parsear en cada render para pintar la tarjeta del turno. Eso vale para una tarjeta que vive dentro de su turno; no vale para un registro que cruza los turnos de la sesión entera, que es lo que esta sección pide. Se añade un slice propio (`registroDeActuaciones`) cuyos únicos escritores son esos tres manejadores. Marcador y registro son **dos vistas del mismo hecho**, no dos hechos: por eso el manejador escribe los dos y por eso una herramienta que se resuelve **cierra su entrada** en vez de añadir una nueva — si añadiera, el registro diría que el agente actuó dos veces en el mundo, que es justo la mentira que este efecto existe para no contar.
> 2. **Se conservan las últimas 20 actuaciones.** El registro enseña una y cuenta el resto; guardar las mil de una sesión maratón sería memoria que nadie va a mirar.
> 3. **El registro se borra al cambiar de cuenta.** Lo que un agente hizo en el mundo por cuenta de un usuario no se le enseña al siguiente que abra el navegador.
> 4. **Va como franja bajo la cabecera, no dentro de ella.** La cabecera del panel tiene alto fijo (`h-16`) y el registro es una línea más. Va **encima** de la tira de pestañas porque lo que registra no pertenece a ningún documento en concreto, sino a la sesión. Con el registro vacío la franja no ocupa nada.
> 5. **Lo entregado es la versión de una línea**, que esta misma sección declara caso base («en 390px una cinta de 3 entradas robaría al transcript exactamente el espacio que el debate necesita»), con el contador con rodillo de §8.12 al lado. La cinta de 3 entradas de `lg+` y el sheet del registro completo al tocarlo quedan pendientes.
> 6. **La región viva se monta vacía desde el principio.** Varios lectores no anuncian una región que aparece a la vez que su contenido — la misma lección que ya está escrita en la región del recuento de la junta.
